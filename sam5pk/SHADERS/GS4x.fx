/* ############################################################################################
   ...
   GS4X shader - Copyright (C) 2005 guest(r) - guest.r@gmail.com
...

{
	...
	half sd1 = min(dot(abs(i1-i3),dt),0.6*(ko1+ko3));
	half sd2 = min(dot(abs(i2-i4),dt),0.6*(ko2+ko4));	

	float4 w = float4(sd2,sd1,sd2,sd1); 
